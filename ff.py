...
# All sequential cell types in Yosys (from discussion #4124)
FF_TYPES = [
    # Word-level
...
def get_counts(yosys_bin, artifact_path, flow):
    """Get FF and total cell counts using select -count."""
    synth_cmd = "synth -flatten -noabc" if flow == "flatten" else "synth -noabc"
    
    # Build select expression for all FF types
...
def main():
    ...
    for design, tag in designs_to_run:
        ...
        ff, total, output = get_counts(args.yosys, artifact_path, args.flow)
    ...
    if args.csv:
        ...
        for r in results:
            print(f"{r['design']},{r['ff']},{r['logic']},{r['total']},{r['ratio']:.4f}")
    else:
        for r in results:
            pct = r["ratio"] * 100
            print(f"{r['design']}: {r['ff']}/{r['total']} FF ({pct:.1f}%)")
